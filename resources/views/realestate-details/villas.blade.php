<div class="tab_content">

    <div class="apartments_divactive ">
        <div class="container">
            <div class="tab_title" style="color: {{ $realestate->secondary_color }}">
                {{-- <h2>فيلا</h2>
                <p>{{ $realestate->name }}</p> --}}
                
                <p>يـحتوي {{ $realestate->name }} على {{ $realestate->villasCount() }} فلل</p>
            </div>
            <div class="" id="main-floor-box-{{ $realestate->_id }}">
                <div class="" id="#{{ $realestate->_id }}">
                    <div class="Apartments_list" style="background: {{ $realestate->secondary_color }}">
                        <div class="Apartments_list_top">
                
                
                            <h3>{{ $realestate->name }}</h3>
                            <ul class="Apartments_nav">
                
                                @foreach ($apartmentStatuses as $apartmentStatus)
                                    <li @foreach ($realestate->colors as $color)
                                        @if($color['colors_id'] == $apartmentStatus->_id)
                                            style="background:{{ $color['color'] }};"
                                        @endif
                                    @endforeach >{{ $apartmentStatus->name }}</li>
                                @endforeach
                
                            </ul>
                        </div>
                        <div class="Apartments_list_row tabap   tab-active">
                            @foreach ($realestate->villas() as $villa)
                                <div class="Apartments_list_box"  data-id="#{{ $villa->_id }}"
                                    @if (isset($realestate->colors) and $realestate->colors != null)
                                        @foreach ($realestate->colors as $color)
                                            @if($color['colors_id'] == $villa->apartmentStatus->_id)
                                                style="background:{{ $color['color'] }};"
                                            @endif
                                        @endforeach
                                    @endif
                                    >
                                    <div>
                                        <ul>
                                            <li>
                                                <p>رقم الفيلا</p>
                                                <span>{{ $villa->number }}</span>
                                            </li>
                                            <li>
                                                <p>عدد الغرف</p>
                                                <span>{{ $villa->rooms }}</span>
                                            </li>
                                            <li>
                                                <p>المساحة</p>
                                                <span><sub>
                                                        {{-- <img src="{{ asset('assets/images/sign-icon.png') }}" alt=""> --}}
                                                    </sub>{{ $villa->space }}</span>
                                            </li>
                                            <li>
                                                <p>القيمة</p>
                                                <span>ريال {{ $villa->price }}</span>
                                            </li>
                                        </ul>
                                        <span class="Apartments_text">
                                            {!! $villa->description !!}</span>
                                    </div>
                                    <i class="floor_img">
                                        <img src="{{ asset('assets/images/floor_img-1.png') }}">
                                    </i>
                                </div>
                            @endforeach
                        </div>
                
                    </div>
                    <div class="Apartments_msg">
                        <ul>
                            <li>المساحات تشمل الأجزاء الخاصة*</li>
                            <li>الأسعار لاتشمل ضريبة الـتـصـرفـات الـعـقـارية*</li>
                        </ul>
                    </div>
                
                    @foreach ($realestate->villas() as $villa)
                        <div class="Ground_FloorApartment" id="#{{ $villa->_id }}">
                            <div class="Ground_FloorApartment_top">
                                <a></a>
                                <div>
                                    <ul>
                                        <li>مواصفات ومخطط</li>
                                        <li>الفيلا</li>
                                    </ul>
                                    <h3 style="color: {{ $realestate->secondary_color }}">الفيلا ({{ $villa->number }})-({{ $villa->apartmentStatus->name }})</h3>
                                </div>
                            </div>
                            
                            {{-- @include('modals.request-apartment',['apartment'=>$villa]) --}}
                            @include('modals.request-villa',['villa'=>$villa])
                            <div class="floor_info" >
                                <div class="floor_info_row active" style="border-color: {{ $realestate->main_color }};background-color: {{ $realestate->main_color }};">
                                    @if($villa->villaStatus_id =='6374be4d0bb191153a019ad2' and isset($villa->villaStatus_id) )
                    
                                    <div>
                                        <p>انا مهتم  </p>
                                        
                                        <button  data-toggle="modal" data-target="#villaRequest-{{ $villa->_id }}">إرسال</button>
                                    </div>
                                    @else
                                    <div>
                                    
                                    <p>   مباع   </p>
                                    </div>
                                    @endif  
                                </div>
                                <div class="floor_info_row" style="border-color: {{ $realestate->main_color }};color: {{ $realestate->main_color }};">
                                    <div>
                                        <p>:السعر</p>
                                        <strong> {{ $villa->price }} </strong>
                                        <span>ريال</span>
                                    </div>
                                </div>
                                <div class="floor_info_row" style="border-color: {{ $realestate->main_color }};color: {{ $realestate->main_color }};">
                                    <div>
                                        <p>:اجمالي مساحتها</p>
                                        <strong>{{ $villa->space }}</strong>
                                    </div>
                                </div>
                            </div>
                            <p class="transaction_tax">الـسـعـر لايـشمـل ضـريبـة الـتـصـرفـات العقـارية</p>
                
                            @if (isset($villa->advantage_values) and $villa->advantage_values != null)
                                <div class="The_facilities">
                                    <h3 class="The_facilities_title" style="color: {{ $realestate->main_color }};">المرافق</h3>
                                    <div class="Project-Features">
                                        @foreach ($villa->advantage_values as $advantage)
                                            <div class="Project-Features-row" style="background: {{ $realestate->main_color }}">
                                                <h3>{{ $advantage['advantage_name'] }} </h3>
                                                <i style="background: {{ $realestate->secondary_color }}"><img src="{{ asset($advantage['image']) }}"></i>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            @endif
                            @if (isset($villa->image) and $villa->image != null)
                                <div class="Unity-Planner" style="color: {{ $realestate->main_color }};">
                                    {{-- <h3>مـخـطــط الـوحـــدة</h3> --}}
                                    <div class="Unity-Planner_img" style="border-color: {{ $realestate->secondary_color }};">
                
                                        <img src="{{ asset($villa->image) }}">
                
                
                                    </div>
                                </div>
                            @endif
                        </div>
                    @endforeach
                </div>
            </div>
            <div class="floor-box" id="floor-box-{{ $realestate->_id }}">
               
            </div>
        </div>

    </div>
  

</div>
